...
async function syncUsers(interaction, client, bachelorRoles, masterRoles) {
    ...
    try {
        const response = await fetch('https://localhost:7059/api/User/DiscordSync');
        if (!response.ok) {
        ...
        // Get all Discord IDs from the guild
        const allGuildMembers = await interaction.guild.members.fetch();
        const allDiscordIdsInGuild = allGuildMembers.map(member => member.user.id);

        // Check for Discord IDs in the guild that are not in the API response
        const missingDiscordIds = allDiscordIdsInGuild.filter(guildId => !data.some(student => student.DiscordId === guildId));

        // Remove roles for missing Discord IDs
        missingDiscordIds.forEach(missingId => {
            getGuildById(interaction.guild.id, client).then(myguild => {
                removeRoles(missingId, bachelorRoles, myguild); 
                removeBachelorRoles(missingId, myguild); 
            });
        });
